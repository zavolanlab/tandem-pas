...
my %transcripts     = (); # transcript_id to list of exons mapping
my %genes           = (); # gene_id to list of transcript mapping
my %T2G             = (); # transcript_id to gene_id mapping
my %geneCoordinates = ();
my %support_level   = (); # transcript_id to support level
my $NOT_GOOD        = 100; # for transcript support level, this is bad
while ( my $line = <$fh> ) {
  next if ( $line =~ m/^#/ );
  chomp $line;
  my @F = split( /\t/, $line );
  ...
  if ( $F[8] =~ m/transcript_id\s+"([^"]+)";/ ) {
    $transcript_id = $1;
    $transcript_id =~ s/\.\d+$//;
    #save also the support level 
    $support_level{$transcript_id} = $NOT_GOOD;
    if($F[8] =~ m/transcript_support_level\s+"([^" \(]+)";/) {
	    my $num = $1;
	    if($num =~ /\d/) {
	      $support_level{$transcript_id} = $num;
	    }
    }
  }
...
my %clusters = (); # dictionary of PAS cluster entries indexed by chr-str combination
my %sortIdx  = (); # dictionary enforcing an output order (based on the order in which chr-str combinations appear in the PAS cluster file)
my $sortN    = 0; # rank of appearance of the cluster in the PAS cluster file
$n = 0;
while ( my $line = <$fhCL> ) {
  #old line format:
  #chr start stop ID(chr:str:representative:anno) score strand posRelToPAS geneID
  #new line format
  #chr start stop ID(chr:representative:str) totalTPM strand %supportingSamples nrProtocols totalTPM anno(intron/exon/etc.) posRelToPAS
  next if ( $line =~ m/^#/ );
  chomp($line);
  my @F = split( /\t/, $line );
...
foreach my $t ( keys %transcripts ) {
  ...
  #label each genomic position with the number of the exon
  ...
  # start and stop of the transcript is extended by x nts (e.g. 100 nts)
...
if ($nonredundant) {
  ...
  foreach my $key ( keys %out ) {
	...
	# example structure of this array
	# 19 43840080 43840085 19:43840082:+ 0.0326 + 1 3 ENSMUST00000099413:1:1:43838803:43840728 ENSMUSG00000025194 0 nan
    ...
    #iterate over all exonIDs of current chr:strand
    # sorting is done by the following rules
    # 1. decreasing number of tandem pAs
    # 2. if equal,  position of the exon within the transcript (prefer more distal exons)
    # 3. if equal, transcript support level (lower is better)
    # 4. if equal, alphabetical to make sure that the sorting is deterministic
    foreach my $groupid (
      sort { $G{$b}->[0]->[7] <=> $G{$a}->[0]->[7] 
		 or $G{$a}->[0]->[10] <=> $G{$b}->[0]->[10] 
	or $support_level{get_tid($G{$a}->[0]->[8])} <=> $support_level{get_tid($G{$b}->[0]->[8])}
	or get_tid($G{$a}->[0]->[8]) cmp get_tid($G{$b}->[0]->[8])}
      keys %G
      ) {
...
sub get_tid {
	...
	die "Cannot find exon id in $exon\n";
    }
}
